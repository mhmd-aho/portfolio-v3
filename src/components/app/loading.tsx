...
export function Loading({setRenderPage}: {setRenderPage: (value: boolean) => void}) {
    ...
    return (
        <div className='h-screen w-full fixed inset-0 grid grid-cols-2 grid-rows-3 z-50 overflow-hidden' ref={container}>
            <h1 className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-background text-xs min-[380px]:text-sm sm:text-lg md:text-2xl lg:text-4xl font-bold w-full px-4 text-center whitespace-nowrap sm:whitespace-normal scrambleText font-default">WELCOME TO MOHAMAD ABO HAMOUD PORTFOLIO</h1>
